...
CTable::CTable(CTable& pcLeft, CTable& pcRight)
{
	*this = pcLeft + pcRight;
}
//CTable::CTable(CTable& pcOther)
...
void CTable::vPrintUsage(string sUsage)
{
	cout << "\"" << sUsage << " " << "`" << s_name << "`\"" << endl;

}//void CTable::vPrintUsage(string s_usage)

void CTable::vCopyTable(int& iTable)
{
	memcpy(pi_table, &iTable, i_length * sizeof(pi_table));
...
{
	CTable c_to_return(this->s_name + pcOther.s_name, this->i_length + pcOther.i_length);
	for (int ii = 0; ii < i_length; ii++)
	{
		c_to_return.pi_table[ii] = this->pi_table[ii];
	}
	for (int ii = this->i_length; ii < this->i_length + pcOther.i_length; ii++)
	{
		c_to_return.pi_table[ii] = pcOther.pi_table[ii-this->i_length];
	}
	return c_to_return;
}
//void CTable::vShowTable()
